...
class Model_PostManager
{
	...
	function getNextPost()
	{
		...
		$content = str_replace('__IMAGES__', URL::base()."assets/images/posts", $content);
		...
		if(!$this->onePost)
		{
			...
			if($cutPos)
			{
				...
				$content = str_replace('<!-- pagebreak -->', ' ... <a href="'.URL::base().'posts/id/'.$post['id'].'">Read more.</a>', $content);
	...
	function getPagedLinks()
	{
		...
		if(!$this->onePost)
		{
			...
			{
				$linksString .= '<a href="'.URL::base().'posts/page/'.($this->currentPage).'">« Précédent</a>';
			}
			for ($i=0; $i < $this->pageCount; $i++)
			{
				...
				{
					$linksString .= '<a href="'.URL::base().'posts/page/'.($i+1).'">'.($i+1).'</a>';
				}
			}
			...
			{
				$linksString .= '<a href="'.URL::base().'posts/page/'.($this->currentPage+2).'">Suivant »</a>';
			}
			$linksString .='</nav>';
		}
		return $linksString;
