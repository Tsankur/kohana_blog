...
function editPost()
{
	$.ajax(baseUrl+"ajax/getpost?id="+postId, {async: false}).done(function(result){
		$('#title').val(result['title']);
		var content = result['content'];
		content = content.replace('__IMAGES__', baseUrl+"assets/images/posts");
		...
		$("#tagSelected").empty();
		$.ajax(baseUrl+"ajax/gettags?id="+postId).done(function(result){
			if(result)
			{
...
function deletePost()
{
	$.ajax(baseUrl+"ajax/deletepost?id="+$(this).attr('data')).done(function(result){
...
function sendTag(tagName)
{
	if(tagName)
	{
		$.ajax(baseUrl+"ajax/sendtag?name="+tagName).done(function(result){
			if(result)
			{
...
/*function deleteTag()
{
	$.ajax(baseUrl+"ajax/deletetag?id="+$(this).attr('data')).done(function(result){
		$("tr[data_id="+result+"]").remove();
		if(postId == result)
		{
			$("#newPostForm")[0].reset();
...
$(function(){
	...
	$("#submitPost").on("click", function(e)
	{
		if(postId > 0)
		{
			...
			content = content.replace(baseUrl+"assets/images/posts", '__IMAGES__');
			content = content.replace("../../assets/images/posts", '__IMAGES__');
			content = content.replace("../assets/images/posts", '__IMAGES__');
			$("tr[data_id="+postId+"]>td:nth-child(2)").html(title);
			console.log('update');
			$.ajax(baseUrl+"ajax/sendpost?id=" + postId, {method: 'post', data: {'title': title, 'content':  content}}).done(function(result){
				...
				$.ajax(baseUrl+"ajax/edittagrelationship?id=" + postId, {method: 'post', data: {'tags': JSON.stringify(tags)}
				});
			});
		}
		else
		{
			...
			content = content.replace("../assets/images/posts", '__IMAGES__');
			console.log('add');
			$.ajax(baseUrl+"ajax/sendpost", {method: 'post', data: {'title': title, 'content':  content}}).done(function(result){
				alert(result['message']);
				$("#postSectionTitle").html("Edit Post #"+result['postID']);
				$.ajax(baseUrl+"ajax/getpost?id="+result['postID']).done(function(result){
					var tags = new Array();
					tags['toAdd'] = new Array();
					//newPostTags;
					$.ajax(baseUrl+"ajax/edittagrelationship?id="+result['postID'], {method: 'POST', data: {'tags': JSON.stringify(tags)}});
	...
	$(".template").on("click", function(e){
		...
		$.ajax(baseUrl+"ajax/changetemplate?templateName="+$("p:first-child", $(this)).html()).done(function(result){
		});
	});
	//images 
	...
	$("#uploadImageForm").submit(function(event) {
		...
		if(files)
		{
			...
			$.ajax(baseUrl+"ajax/uploadimage", {
				...
				$(".imageSelect, #Images>div").append('<img src="'+baseUrl+'assets/images/posts/'+result+'"/>');
